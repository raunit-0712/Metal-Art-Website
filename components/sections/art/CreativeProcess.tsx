...
export function CreativeProcess() {
  return (
    <section className="py-24 md:py-32 bg-[#FFFFFF] border-t border-[#E8E4DD]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, index) => (
            <SectionReveal key={step.title} delay={0.15 * index}>
              <motion.div
                whileHover={{ y: -4 }}
                className="relative text-center p-8 bg-[#FCFBF8] border border-[#E8E4DD] rounded-[16px] shadow-[0_10px_30px_rgba(13,19,33,0.05)] transition-all duration-300"
              >
                {/* Step Number */}
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 font-playfair text-8xl text-[#111111]/5 select-none">
                  {index + 1}
                </div>

                <div className="relative z-10">
                  <div className="w-16 h-16 rounded-full bg-[#B08D57]/10 group-hover:bg-[#B08D57]/20 transition-all duration-500 flex items-center justify-center mx-auto mb-6">                    <step.icon size={32} className="text-[#B08D57]" />
                  </div>
                  <h3 className="font-playfair text-2xl text-[#111111] mb-4">
                    {step.title}
                  </h3>
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-16 right-0 w-full h-px">
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1/2 h-px bg-gradient-to-r from-[#B08D57]/20 to-transparent" />
                  </div>
                )}
              </motion.div>
            </SectionReveal>
